...
\subsubsection {Method of moments estimator}
 ...
 we belive if for any function h(x), $\int h(x)f_\theta(x)dx = \int h(x) f_{\theta^*}(x)dx  = E_{\theta^*}[h(x)]$, then $f_\theta(x)$ is a good approximation of  $f_{\theta^*}(x)$. But obviously we cannot test all possible $h(x)$, so we use only polynormals  (according to \textbf{weiersstress approximation theorem}, any continuous function can be arbitrarily well approximated by polynormials), and we choose $x^k$ because any polynormials can be linearly combined by  $x^k$ where $ (k \in N)$. The last problem is what the upper bound of $k$ should we choose. as a rule of thumb, if $\theta \in R^d$, we need d moments.
 
 For discrete distributions, say, if there are $r$ possible values, $E=\{x_1,x_2,...x_r\}$, the PMF has $r-1$ unknowns, we could solve PMF with r-1 moment equations and $\sum p(x_i)=1$, this is multivariate linear problem. Written in matrix form we get a vandermonde matrix, which is invertable, and then we are assured a solution. (the determinant of vandermonde matrix is $\sum_{1 \le i < j \le r}(x_i-x_j)$, since x are possible values of a random variable, they are different by difinition, so the determinant is non zero.) Note that this means for any distribution, we can use method of moments to determine its PMF (without any parameter), just by inverting a vandermonde matrix. so in most cases, if we use parameteric model, we need much less moments to estimate the parameters. And also note that here comes the numerical skills, if the inverse of the vandemonde matrix has large \textbf{condition number}, then the esitmated parameter $\theta$ has high variance,i.e. not reliable. The condition number is the largest eigen value of the matrix.
 
 in one word, if we can accurately estimate moments, then we may be able to recover pdf or pmf, So, how do we esitmate the k-th moment? Given n sample, $\hat{m}_k= (1/n)\sum_{i=1}^n x_i^k$, this is called empirical moments.

\subsubsection {MLE}

MLE is introduced by R.A.Fisher in 1912.

minimize the tatal variant distance when we build an estimator is the ultimate target, but it is hard if not impossible to compute it, so we use Kullback Leibler divergence, which has some good properties for our purpose. note that total variant distance is truely a distance metric, while KL is not (so it is called divergence). 

If we want to esitmate the parameter $\theta^*$ of one distribution P, denoted as $\hat \theta$, the more close the two distribution $P_\theta^*$ and $P_\hat{\theta}$ the better. then how we measure the closedness of two distributions? we use total variant distance, 
$$TV(P_\theta^*, P_\hat{\theta}) = \max_A{|P_{\theta^*}(A) - P_\hat{\theta}(A)|}$$ 
where A represent some event. our stratege is to construct an esitmator $$\hat{TV}(P_\theta, P_{\theta^*})$$
and then find $$\arg\min_\theta [\hat{TV}(P_\theta, P_{\theta^*})]$$

????????????????????????$\theta^*$????????A??????????????????
?????KL divergence, ??KL?????????$KL(P,Q) \neq KL(Q,P)$????KL(P,Q)=0??P=Q?
?????Estimator????KL???????$$\theta$$, $$\arg\min_\theta[KL(P_{\theta^*}, P_\theta)]$$
...
??KL?????????????????2?????([cross entropy](https://en.wikipedia.org/wiki/Cross_entropy))????????????????????

?: ????: $$\arg\max (\log a+\log b) = \arg\max \log ab = \arg\max ab$$

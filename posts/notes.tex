...
VI interpretted with Expectation is easier to understand.
...
D is used to normalize the  exp then to make it a probability density of $z_j$. typically $$ D = \int exp([E_{q_{-j}} [\log p(z_j, z_{-j}, x)]]) dz_j$$
...
\begin{itemize}
	...
	continuous map - inverse image of open set is open.
	
	homoemorphism - continuous bijective map with continuous inverse
	...
	locally finite: a collection A of subsets of X, is locally finite, if for all x in X, x has a neighborhood that intersect at most finitely many of sets in A. (lie: if A is finite, A is locally finite. infinite collections can also be locally finite, e.g. $A=\{(n,n+2)|n \in Z \}$, a countable collection of subsets need not be locally finite, e.g. (-n, n) with integer n.)
